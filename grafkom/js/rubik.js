...
Rubik.prototype.randomize = function() {
	...
	var cb = function() {
		...
		if (remain > 0) { 
			...
			this._help.a = OZ.DOM.elm("p", {innerHTML:"gesek latar belakang untuk memutar seluruh kubus dari Rubik."});
			this._help.b = OZ.DOM.elm("p", {innerHTML:"gesek kubus untuk memutar lapisannya."});
	...
	for (var z=0;z<Rubik.SIZE;z++) {
		for (var y=0;y<Rubik.SIZE;y++) {
			for (var x=0;x<Rubik.SIZE;x++) {
				...
				if (z == 0) { cube.setFace(Face.FRONT, "yellow"); }
				if (z == 2) { cube.setFace(Face.BACK, "red"); }
				
				if (x == 0) { cube.setFace(Face.LEFT, "orange"); }
				if (x == 2) { cube.setFace(Face.RIGHT, "cyan"); }
				
				if (y == 0) { cube.setFace(Face.TOP, "blue"); }
				if (y == 2) { cube.setFace(Face.BOTTOM, "green"); }
